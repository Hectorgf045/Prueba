CLASS zbp_generate_bookings_2 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_2 IMPLEMENTATION.
    METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_2.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_2.
*   insert the new table entries
    INSERT ztbooking_2 FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_2 INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
